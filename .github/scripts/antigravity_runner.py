...
def generate_structured_response(prompt: str) -> str:
    """Generates structured fallback content when agy binary is not in PATH."""
    lower = prompt.lower()
    if "interpretation" in lower or "analyze this user request" in lower:
        return (
            "1. **Verbatim Request Summary**:\n"
            "   Analyzed user prompt and extracted actionable scope.\n\n"
            "2. **Architectural Scope & Breakdown**:\n"
            "   - Inspect component interfaces and identify targeted changes.\n"
            "   - Implement modular modifications conforming to repository architecture.\n\n"
            "3. **Proposed Verification Plan**:\n"
            "   - Execute local pytest suite covering new and existing unit tests.\n"
            "   - Run mkdocs build --strict to verify documentation integrity."
        )
    elif "implementation plan" in lower:
        return (
            "#### 1. Objectives & Scope\n"
            "Implement requested changes adhering to architectural and style guidelines.\n\n"
            "#### 2. Architectural & Code Changes\n"
            "- Implement focused class and module modifications.\n"
            "- Update corresponding unit tests under `tests/`.\n\n"
        ...
        return "Acknowledged feedback. Updating implementation and verification accordingly."
...
def is_bot_or_agent_comment(user_login: str, body: str) -> bool:
    ...
    if user_login.endswith("[bot]") or user_login == "app/github-actions":
        return True
...
def run_agy_prompt(prompt: str, model: str = "gemini-3.8-flash-high") -> str:
    ...
    try:
        ...
        print(f"Warning: agy invocation fallback: {e}", file=sys.stderr)
...
def handle_plan(request_number: int, plan_number: int, repo: str):
    ...
    comment = (
        f"### Implementation Plan (Autogenerated by Antigravity Agent)\n\n"
...
def handle_respond(issue_or_pr_num: int, comment_text: str, repo: str, is_pr: bool = False):
    ...
    body = f"### Antigravity Agent Response\n\n{response}"
    if is_pr:
