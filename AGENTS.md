...
Pull requests require official GitHub review approval from the user (`marius-patrik`) before merging. Direct self-merges without user review approval are strictly prohibited. The `main` branch protection requires at least 1 approving review (`required_approving_review_count: 1`) without disallowing merge from the last pusher (`require_last_push_approval: false`). Once approved by the user, the pull request may be merged by the last pusher or author.
