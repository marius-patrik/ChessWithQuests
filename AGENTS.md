...
Pull requests require official GitHub review approval before merging. The `main` branch protection requires at least 1 approving review (`required_approving_review_count: 1`) on the last commit before merge (`dismiss_stale_reviews: true`) without disallowing merge from the last pusher (`require_last_push_approval: false`). To resolve GitHub's platform restriction preventing pull request authors from approving their own pull requests, an automated proxy review workflow (`.github/workflows/proxy-approval.yml`) listens for sign-off signals (`approve`, `merge`, `/approve`, `lgtm`) from `marius-patrik` via pull request reviews or comments. Upon receiving user approval, the proxy automation marks the draft PR as ready (`gh pr ready`), submits an approving review as `github-actions[bot]`, and activates auto-merge (`gh pr merge --auto --merge`). Once all CI checks are green and the branch incorporates latest `main`, the pull request merges automatically.
