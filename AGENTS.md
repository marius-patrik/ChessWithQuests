...
Keep commits modular, focused, and descriptive. When implementing components from specifications, maintain one commit per class or component. All commits across all branches must strictly follow the Conventional Commits specification: `<type>(<scope>): <description>` (e.g. `feat(model): implement knight movement rules`).
...
- **Required CI Checks**: Every pull request requires all GitHub Actions CI checks (`CI` matrix across Python versions, `Verify Docs Layout`, `Verify Bound Issue`, and `Auto Format`) to pass green before merging.
- **Branch Protection**: The `main` branch must remain protected at all times with required status checks, branch up-to-date enforcement, and pull request review enforcement.
...
- **Issue Binding**: Every pull request must bind a tracked GitHub issue using closing keywords in the PR description (e.g. `Closes #123`, `Fixes #123`, or `Resolves #123`).
- **Branch Auto-Deletion**: Merging the pull request automatically closes the bound issue, and the remote branch is automatically deleted by GitHub upon merge (`delete_branch_on_merge: true` and `--delete-branch`), and must be pruned/deleted locally.
- **Project Board Status Taxonomy**: All issues and pull requests created are automatically added to GitHub Project 14 with automated status movements:
...
Before starting implementation on any task, the implementation plan must be documented in a dedicated child issue with the `Plan` label natively linked as a sub-issue to its parent `Request` issue (`--parent <request_id>`). The plan must detail objectives, architectural/code changes, and verification steps:
- **Implementation Review Gate**: Prior to merging the bound pull request, an implementation review must be conducted and commented on the child Plan issue confirming that the implementation matches the plan exactly (`Matches Plan: Yes`).
- **Plan Alignment**: If the implementation diverged from the plan, an alignment comment (`Plan Alignment:`) detailing all deviations must be posted and explicitly approved before the pull request can be merged. Automated CI checks enforce the presence of both the plan and the pre-merge review on all bound issues.

### 11. Pull Request Review Approval Enforcement & Auto-Merge
Pull requests require official GitHub review approval before merging. The `main` branch protection requires at least 1 approving review (`required_approving_review_count: 1`) on the last commit before merge (`dismiss_stale_reviews: true`) without disallowing merge from the last pusher (`require_last_push_approval: false`):
- **Native Approval**: Because PRs are authored by `github-actions[bot]`, `marius-patrik` can natively review and select **Approve** in the GitHub UI (or comment `/approve`, `approve`, `lgtm`, or run `gh pr review <id> --approve`).
- **Auto-Merge Activation**: An automated workflow (`.github/workflows/pr-approval-automerge.yml` and `.github/scripts/handle_pr_approval.py`) listens for review approvals from `marius-patrik`. Upon receiving user approval, the automation marks the draft PR as ready (`gh pr ready`), submits proxy approval if required, and activates auto-merge with branch auto-deletion (`gh pr merge --auto --merge --delete-branch`). Once all CI checks are green and the branch incorporates the latest `main`, the pull request merges automatically.
- **Post-Merge Reconciliation**: Upon merge, the automation directly updates Project 14 statuses for both the PR and all bound issues to `Done`, adds the `Done` label, and verifies bound issues are closed.
...
- **Decomposition**: A single user message containing multiple distinct tasks must be decomposed into multiple focused `Request` issues.
- **Verbatim Wording**: Each `Request` issue body must contain the exact, verbatim wording of the user request.
- **Interpretation Section**: Below the verbatim wording, each `Request` issue must include an `### Interpretation` section clearly specifying how the request is understood, the architectural scope, and proposed verification.
