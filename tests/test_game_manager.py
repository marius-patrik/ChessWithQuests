...
def test_game_manager_initialization():
    gm = GameManager()
    assert gm.aktivni_hrac == 1
    assert len(gm.hraci) == 2
    assert gm.get_stav() == GameManager.STATE_IN_PROGRESS
...
def test_game_manager_make_move_and_switch_turns():
    ...
    assert gm.aktivni_hrac == -1
    assert gm.aktualni_tah is move
...
def test_game_manager_timeout_state():
    gm = GameManager()
    gm.casovac.cas_hrac[0] = 0  # White timed out
    assert gm.get_stav() == GameManager.STATE_TIMEOUT
