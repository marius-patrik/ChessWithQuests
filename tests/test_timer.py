...
def test_timer_reset_and_increment():
    ...
    timer.nuluj_cas()
    assert timer.cas_hrac == [100, 100]


def test_timer_expired():
    timer = Timer(5)
    assert not timer.is_expired(1)
    timer.pocitej_cas(1, 10)
